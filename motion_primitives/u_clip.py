...
from cable_routing.debug_gui.backend.board_projection import (
    pixel_from_world_debug,
    world_from_pixel_debug,
)
...
def build_u_clip_entry_pixels(
    ...
    Distances are configured in meters and projected back into image pixels for
    the overlay and later world-target generation.
    """
    ...
    forward_probe_px = center + forward
    forward_probe_world = world_from_pixel_debug(
        state.env,
        state.config,
        (float(forward_probe_px[0]), float(forward_probe_px[1])),
        arm=primary_arm,
        is_clip=True,
        image_shape=state.rgb_image.shape,
    ).reshape(3)
    ...
    clip_forward_world = clip_forward_world / norm
    ...
    primary_uv = pixel_from_world_debug(
        state.env,
        state.config,
        primary_world,
        arm=primary_arm,
