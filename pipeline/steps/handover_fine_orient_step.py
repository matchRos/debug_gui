...
class HandoverFineOrientStep(BaseStep):
    """
    At the lifted-grasp position only: apply small tool-frame rotations on top of the
    current grasp orientation (few degrees — config). Does not move the TCP to the
    exchange point; run ``handover_move_exchange`` next.
    """
    ...
    def __init__(self) -> None:
        ...
        if not rospy.core.is_initialized():
            rospy.init_node("debug_gui_handover_fine_orient", anonymous=True)
    ...
    def run(self, state: PipelineState) -> Dict[str, Any]:
        ...
        if ridx < 0 or ridx >= len(state.routing):
            raise RuntimeError(
                f"handover_clip_routing_index {ridx} invalid for routing length {len(state.routing)}."
            )
        ...
        pos_lift = np.asarray(gpose["position"], dtype=float).reshape(3) + lift_offset_along_plane_normal(
            plane, lift_m
        )
        ...
        msg, quat = pose_to_msg(pos_lift, R, config=state.config)
