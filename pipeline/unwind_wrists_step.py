...
class UnwindWristsStep(BaseStep):
    ...
    def __init__(self):
        ...
        if not rospy.core.is_initialized():
            rospy.init_node("debug_gui_unwind_wrists", anonymous=True)
    ...
    def _find_joint_index(self, joint_state: JointState, candidates):
        for cname in candidates:
            if cname in joint_state.name:
                return joint_state.name.index(cname), cname
        return None, None
    ...
    def run(self, state: PipelineState) -> Dict[str, object]:
        ...
        target_speed = np.deg2rad(25.0)  # rad/s
        duration_s = 1.2
