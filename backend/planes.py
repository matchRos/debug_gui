...
class RoutingPlane:
    ...
    def from_config_entry(entry: Dict[str, Any]) -> "RoutingPlane":
        ...
        v_axis = np.asarray(entry.get("v_axis", [0.0, 1.0, 0.0]), dtype=float).reshape(3)
...
def get_routing_plane(config: Any, clip_id: Optional[int] = None) -> RoutingPlane:
    if config is None:
        raise RuntimeError("DebugConfig not available.")
...
def point_at_plane_height(
    ...
    p_plane = project_to_plane(point_world, plane)
    return p_plane + float(height_above_plane_m) * plane.normal
...
def ensure_min_plane_height(
    ...
    p = np.asarray(point_world, dtype=float).reshape(3)
    signed = float(np.dot(p - plane.origin, plane.normal))
