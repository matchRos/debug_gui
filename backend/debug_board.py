...
class DebugClip:
    """
    Minimal standalone clip representation for the new debug pipeline.
    """
...
class DebugBoard:
    """
    Minimal standalone board representation.

    This avoids importing the old board implementation and therefore also
    avoids dependencies on tyro / old config plumbing.
    """
    ...
    def _load_config(self, config_path: str) -> Dict:
        ...
        if not isinstance(data, dict):
            raise ValueError(
                "Board config JSON must contain a dictionary at the top level."
            )
